...
namespace Rendering
{
	...
	ObjectDiffuseLight::ObjectDiffuseLight(Game& game, Camera& camera, std::string filePath)
	...
	void ObjectDiffuseLight::Initialize()
	{
		...
		std::unique_ptr<Model> model(new Model(*mGame, fileName, true));
		//std::unique_ptr<Model> model2(new Model(*mGame, "Content\\Models\\HepBurn_Sofa.3ds", true));
